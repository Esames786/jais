...
if (!empty($_POST['ven_p_pname']) || !empty($_POST['ven_p_pdesc']) || $_FILES['ven_p_ppic1']) {
    ...
    if (in_array($ext, $valid_extensions)) {
        ...
        if (move_uploaded_file($tmp, $target)) {
            $sql = "SELECT * FROM package WHERE s_product_name ='$ven_p_pname'";
            $result = $connect->query($sql);
            if ($result->num_rows == 0) {
                $que = "INSERT INTO package(s_product_name,s_product_desc,s_product_image,category_name) VALUES ('$ven_p_pname','$ven_p_pdesc','$ven_p_ppic1','$ven_p_pcatagory')";
